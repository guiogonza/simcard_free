...
def index():
    ...
    # NUEVO: última actualización a mostrar en la tarjeta Red Móvil
    last_updated = None
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if data1.get("meta", {}).get("count", 0) <= 0:
                error_message = "No se encontró SIM con el ICCID proporcionado."
            else:
                sim_id = data1["data"][0]["id"]
...
def sims_refresh_specific():
    try:
        ...
        if not iccids:
            return jsonify({"ok": False, "error": "iccids vacío"}), 400
